...
class SentDesign extends StatefulWidget {
  final Map details;
  SentDesign({required this.details});
  @override
  _SentDesignState createState() => _SentDesignState();
}

class _SentDesignState extends State<SentDesign> {
      ...
      children: [
       widget.details["type"] == "files" ?
         ...
         Container(
          ...
          child: ImagesMessages(true, jsonDecode(widget.details["message"]),widget.details),
        ) :
        GestureDetector(
          child: Container(
            padding: EdgeInsets.only(left: 70,right: 20,bottom: 8),
            width: double.infinity,
            alignment: Alignment.centerRight,
            child: Container(
              ...
              child: Text(widget.details["message"],style: TextStyle(color: Colors.black),),
            ),
          ),
          onLongPress: (){
        ...
        Padding(
          ...
          child: DateFormat.yMMMd('fr_FR').format(DateTime.parse(widget.details["created_at"].toString())) == DateFormat.yMMMd('fr_FR').format(DateTime.now().toUtc().add(Duration(hours: 2))) ?
          Text("Aujourd'hui, ${DateFormat("HH:mm",'fr_FR').format(DateTime.parse(widget.details["created_at"].toString()).toUtc().add(Duration(hours: 1)))}",style: TextStyle(fontSize: 11,color: Colors.grey),) :
          Text(DateFormat('fr_FR').format(DateTime.parse(widget.details["created_at"].toString())),style: TextStyle(fontSize: 11,color: Colors.grey),),
        ),
        SizedBox(
          height: 15,
        ),
